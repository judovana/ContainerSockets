...
echo "COPY res res " >> $preparation_dockerfile
echo "RUN pwd" >> $preparation_dockerfile
echo "RUN ls -l ." >> $preparation_dockerfile

podman build --tag run-incont-xerces -f ./$preparation_dockerfile

podman run --name xerces run-incont-xerces bash runlocal.sh
#podman run -it --name xerces run-incont-xerces bash
